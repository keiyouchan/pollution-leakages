...
* --------------------
use code year 所有制 using  "$raw/firm_correlates_raw.dta" , clear
rename 所有制 ownership
tempfile ownership
save `ownership' , replace 

/* generate SOE indicator */
use  "firm_reg_balance.dta" , clear
gen   post = year >= 2014
merge m:1 code year using `ownership' , keep(1 3) nogen
gen   soe = ownership == "国企"
...
preserve 
	...
	foreach Y in $outcomes {
			...
			* output
			...
			using "$hetero/soe_`Y'.csv", ///
			b(4) se(4) ///
			keep(policy) order(policy) ///
			r2(4) star(* 0.1 ** 0.05 *** 0.01) replace
	}
restore
			reghdfe stock_out policy , a($hdfe_dummy)   vce(cluster $clusters)
...
twoway (rcap lb ub y_index if  model_index == 3 & group == 1 , 
       ...
       graph export "$hetero/by_ownership.png" , replace ;
#delimit cr


/* interaction */
gen interaction = policy * soe
...
foreach Y in $outcomes {
		...
		* output
		esttab model1 model2 model3 model4 model5  ///
		using "$hetero/soe_interaction_`Y'.csv", ///
		b(4) se(4) ///
