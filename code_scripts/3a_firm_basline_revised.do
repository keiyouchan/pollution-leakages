...
/* main setting */
* outcome variables
global outcomes "stock_out netinc_out inc_out firmnum_out lnstock_out lninc_out lnfirmnum_out"
	* model
	global hdfe       "code year provid#year indusid#year"
	global hdfe_dummy "code year provid#year indusid#year $citycorr"
	global clusters2 "cityid"
...
global twfe_dummy "code year $citycorr"

global hdfe "code year provid#year indusid#year"
global hdfe_dummy "code year provid#year indusid#year $citycorr"

global clusters1 "code"
global clusters2 "cityid"
...
global model5 "$citycorr $firmcorr"
global allmodels "model1 model2 model3 model4"
...
foreach Y in $outcomes {
	...
	local index = 1
		    ...
		    reghdfe `Y' policy inconti $cityl , a($hdfe) vce(cluster $clusters2)
		    est sto model1
		    reghdfe `Y' policy $cityq , a($hdfe) vce(cluster $clusters2)
		    est sto model2
		    reghdfe `Y' policy $citypoly, a($hdfe) vce(cluster $clusters2)
		    est sto model3
		    reghdfe `Y' policy $citypost, a($hdfe) vce(cluster $clusters2)
		    est sto model4
			reghdfe `Y' policy , a($hdfe_dummy) vce(cluster $clusters2)
			est sto model5
...
foreach Y in $outcomes {
	...
	foreach model of global allmodels {

		    local model_content = `"$`model'"' 
		    reghdfe `Y' F2-F4 L0-L3 `model_content', a($hdfe) vce(cluster $clusters2)

		    est sto model`index'
		    local index = `index' + 1
	}
			reghdfe `Y' F2-F4 L0-L3 , a($hdfe_dummy) vce(cluster $clusters2)
			est sto model5
			...
			* output
			...
			/*
			coefplot model1 model2 model3 model4 model5 , ///
					keep(F2-F4 L0-L3) ///
					aseq swapnames ///
					noeqlabels ///
					vertical ///
...
reghdfe lninc_out policy c.roa_2010#c.year#c.year#c.year c.roa_2010#c.year#c.year c.roa_2010#c.year, a($hdfe) vce(cluster $clusters2)
